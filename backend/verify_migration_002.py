            ...
            try:
                ...
                if 'error' in result and result['error']:
                    if 'column' in result.get('message', '').lower():
                        ...
                        print(f"  ⚠️  {column_name}: 查询出错 - {result.get('message', 'Unknown error')}")
    ...
    try:
        ...
        if result.get('data'):
            ...
            for case in result['data'][:5]:
                ...
                is_new_format = bool(case_id and case_id.startswith('PRJ'))
            ...
            print(f"\n📊 格式统计:")
            print(f"   新格式 (PRJ001-MOD001-001): {new_format_count}")
...
def print_summary(structure_results, project_ok, module_ok, case_ok):
    ...
    print(f"\n📊 数据完整性:")
    print(f"  {'✅' if project_ok else '❌'} 项目数据可访问")
    print(f"  {'✅' if module_ok else '❌'} 模块数据可访问")
    print(f"  {'✅' if case_ok else '❌'} 测试用例数据可访问")
