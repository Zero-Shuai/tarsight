...
interface TestCaseFormDrawerProps {
  testCase?: TestCase
  modules: Module[]
  initialDraft?: AITestCaseDraft | null
  onClose: () => void
  onSuccess: () => void
}

export function TestCaseFormDrawer({ testCase, modules, initialDraft, onClose, onSuccess }: TestCaseFormDrawerProps) {
  ...
  const getInitialFormData = () => ({
    ...
    test_name: testCase?.test_name || initialDraft?.test_name || '',
    url: testCase?.url || initialDraft?.url || '',
    method: testCase?.method || initialDraft?.method || 'GET',
    expected_status: testCase?.expected_status || initialDraft?.expected_status || 200,
    description: testCase?.description || initialDraft?.description || '',
    module_id: testCase?.module_id || initialDraft?.module_id || '',
    tags: testCase?.tags || initialDraft?.tags || [],
    headers: parseJsonField(testCase?.headers) || initialDraft?.headers || null,
    request_body: parseJsonField(testCase?.request_body) || initialDraft?.request_body || null,
    variables: parseJsonField(testCase?.variables) || initialDraft?.variables || null,
    // assertions field removed from state - doesn't exist in database schema
    level: testCase?.level || initialDraft?.level || 'P2',
  ...
  useEffect(() => {
    ...
    if (testCase) {
      ...
      setAssertionsConfig(initialDraft.assertions)
    } else {
