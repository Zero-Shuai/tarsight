...
-- Created: 2026-01-06
-- Description: 支持自定义的字母+数字组合编号，不限制为 PRJ/MOD 前缀
-- ============================================================================
...
CHECK (
  project_code ~ '^[A-Za-z0-9]{1,20}$'
  AND project_code ~ '[0-9]'
  AND project_code ~ '[A-Za-z]'
);

COMMENT ON COLUMN public.projects.project_code IS
'项目编号：1-20位字母和数字组合，必须包含至少一个字母和一个数字';
...
CHECK (
  module_code ~ '^[A-Za-z0-9]{1,20}$'
  AND module_code ~ '[0-9]'
  AND module_code ~ '[A-Za-z]'
);

COMMENT ON COLUMN public.modules.module_code IS
'模块编号：1-20位字母和数字组合，必须包含至少一个字母和一个数字';
...
BEGIN
  ...
  WHERE project_code !~ '^[A-Za-z0-9]{1,20}$'
     OR project_code !~ '[0-9]'
     OR project_code !~ '[A-Za-z]';

  -- 检查模块编号
  SELECT COUNT(*) INTO invalid_module_count
  FROM public.modules
  WHERE module_code !~ '^[A-Za-z0-9]{1,20}$'
     OR module_code !~ '[0-9]'
     OR module_code !~ '[A-Za-z]';
...
'测试用例ID：格式为 {项目编号}-{模块编号}-{3位序号}（项目编号和模块编号支持任意字母数字组合）';
